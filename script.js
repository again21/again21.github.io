...
window.onclick = function(event) {
    ...
    //创建b元素
    var effect = document.createElement("b")

    //防止拖动
    effect.onselectstart = new Function("event.returnValue=false")

    //将b元素添加到页面上
    document.body.appendChild(effect).innerHTML = array_list[indices]
    indices = (indices + 1) % array_list.length;

    //给p元素设置样式
    effect.style.cssText = "position: fixed;left:-100%;"
